 ...
 * @updated 2024-03-15
 *
 * This file shows the main menu in the user's panel
 */
...
const SidebarLayout = (): React.JSX.Element => {
    ...
    const dropdownitems: Array<{text: string, url: string}> = [{
        text: getWord('user', 'user_profile', memberOptions['Language']),
        url: '/user/profile'
    }, {
        ...
        <>
            <div className={(memberOptions.SidebarStatus === '0')?'fc-dashboard-sidebar fc-minimized-sidebar':'fc-dashboard-sidebar'}>
                    ...
                    <h4>
                        <Link href={process.env.NEXT_PUBLIC_SITE_URL as unknown as URL}>
                            {( websiteOptions.DashboardLogoSmall && websiteOptions.DashboardLogoLarge)?(
                                <>
                                    <Image src={websiteOptions.DashboardLogoSmall} layout="fill" alt="Small Logo" className="fc-dashboard-sidebar-header-logo-small" onError={(e) => {e.currentTarget.src = '/assets/img/cover.png';}} />
                                    <Image src={websiteOptions.DashboardLogoLarge} layout="fill" alt="Large Logo" className="fc-dashboard-sidebar-header-logo-large" onError={(e) => {e.currentTarget.src = '/assets/img/cover.png';}} />
                                </>                            
                            ):(
                                <>
                ...
                <div className="fc-dashboard-sidebar-body">
                    <ul>
                        <li>
                            <Link href="/user/dashboard" className={(pathname === '/user/dashboard')?'fc-sidebar-item-active':''} scroll={false}>
                        ...
                        <li>
                            <Link href="/user/websites" className={(pathname === '/user/websites')?'fc-sidebar-item-active':''} scroll={false}>
                                { getIcon('IconWebsites', {className: 'fc-dashboard-sidebar-icon'}) }
                                <span className="fc-dashboard-sidebar-menu-item">
                        ...
                        <li>
                            <Link href="/user/threads" className={(pathname === '/user/threads')?'fc-sidebar-item-active':''} scroll={false}>
                                { getIcon('IconThreads', {className: 'fc-dashboard-sidebar-icon'}) }
                                <span className="fc-dashboard-sidebar-menu-item">
                ...
                <div className="fc-dashboard-sidebar-bottom">
                    <Link href="#" className="flex fc-member-picture" onClick={openProfileDropdown}>
                        ...
                        <div className="ml-3">
                            ...
                            <p className="text-sm">{(typeof memberOptions.SubscriptionPlanName != 'undefined')?memberOptions.SubscriptionPlanName:''}</p>
                        </div>
                    </Link>
